...
export default {
  ...
  data() {
    return {
      ticker: '',
      tickers: [],
      apiKey: 'd0888e0ebae6556674d9a40f7e7e66e4565c7b98d1b67835424d7e771c4e67d0'
    }
  },
  methods: {
    addTicker(){
      ...
      const chosenTicker = this.tickers.find(f => f.name === newTicker.name);
      console.log(chosenTicker);

      if (chosenTicker) {
        setInterval( async () => {

          const path = `https://min-api.cryptocompare.com/data/price?fsym=${chosenTicker.name}&tsyms=USD&api_key=${this.apiKey}`;
          console.log(path);

          const responce = await fetch(path);
          const data = await responce.json();
          chosenTicker.price = data.USD;
          console.log(data)
